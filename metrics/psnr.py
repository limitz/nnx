...
def mse(a,b, reduction="mean"):
    r = [p.sub(q).pow(2).mean() for p,q in _prep(a,b)]
    r = _torch.stack(r)
    return _reduce(r, reduction) 

def image_psnr(a,b,reduction="mean"):
    r = [_torch.tensor(20 * _math.log10(255)) - 10 * mse(p,q).log10() for p,q in _prep(a,b)]
    r = _torch.stack(r)
    return _reduce(r, reduction) 

def image_psnr_hvs_m(a,b,wstep=8,reduction="mean"):
    r = [_torch.tensor(_psnrhvsm(p,q,wstep)[0]) for p,q in _prep(a,b)]
    r = _torch.stack(r)
    return _reduce(r, reduction) 

def image_psnr_hvs(a,b,wstep=8,reduction="mean"):
    r = [_torch.tensor(_psnrhvsm(p,q,wstep)[1]) for p,q in _prep(a,b)]
    r = _torch.stack(r)
    return _reduce(r, reduction) 

def _psnrhvsm(img1=None,img2=None,wstep=8):
    verbose = True
    p_hvs_m = 0 
    p_hvs = 0
    ...
    CSFCof=_np.array([[1.608443, 2.339554, 2.573509, 1.608443, 1.072295, 0.643377, 0.504610, 0.421887],
    ...
    MaskCof=_np.array([[0.390625, 0.826446, 1.000000, 0.390625, 0.173611, 0.062500, 0.038447, 0.026874],
    ...
    A = _view_as_blocks(img1, window_shape)
    B = _view_as_blocks(img2, window_shape)
    ...
    #compute the 2d Discrete Cosine Transform
            ...
            '''
            #dct2(A)
            a_dct=_dct(patchA,type=2,axis=1,norm='ortho')
            A_dct=_dct(a_dct,type=2,axis=0,norm='ortho')
            #dct2(B)
            b_dct=_dct(patchB,type=2,axis=1,norm='ortho')
            B_dct=_dct(b_dct,type=2,axis=0,norm='ortho')
            ...
            for k in range(7):
                for l in range(7):
                    ...
                    S2=S2 + ((_np.dot(u,CSFCof[k,l]))**2) #PSNR-hvs
                    if (k != 1) or (l != 1):
                        if u < MaskA / MaskCof[k,l]:
                            u=0
                        else:
                            u=u - (MaskA / MaskCof[k,l])
                    S1=S1 + ((_np.dot(u,CSFCof[k,l]))** 2) # PSNR-HVS-M
                    Num=Num + 1
    ...
    if Num != 0:
        ...
        if S1 == 0:
            ...
            p_hvs_m= 10*(_np.log10(255*(255/ S1)))
            #print('p_hvs_m: {:.0f}'.format(p_hvs_m))
        if S2 == 0:
            #print("p_hvs=100000")
            p_hvs=100000
        else:
            p_hvs= 10*(_np.log10(255*(255 / S2)))
...
def maskeff(z=None,zdct=None,*args,**kwargs):
    ...
    for k in range(7):
        for l in range(7):
            if (k != 1) or (l != 1):
                m=m + _np.dot((zdct[k,l] ** 2),MaskCof[k,l])
...
def vari(AA=None,*args,**kwargs):
    ...
    varia = _np.var(flat)
    #flat_sz = _np.size(flat)
    
    #print(flat)
    #print(varia)
    #print(flat_sz)
    d=_np.dot(varia, flat.size)
    return d
